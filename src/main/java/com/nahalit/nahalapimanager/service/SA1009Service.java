...
public class SA1009Service {
    ...
    public List<SaRegion> getAllRegion() {
        return saRegionRepository.findAll(Sort.by("regionNo").ascending());
    }
    ...
    public List<SaRegion> saveRegionList(List<SaRegion> saRegions) {
        ...
        saRegions.forEach(region -> {
            try {
                region.setSsCreatedOn(UtillDate.getDateTime());
                region.setSsModifiedOn(null);
                saRegionList.add(this.saRegionRepository.save(region));
    ...
    public List<SaSubregion> getAllSubregion(Long regionNo) {
